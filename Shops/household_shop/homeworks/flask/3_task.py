...
def get_day_shedule(schedule_data: dict, weak_day: str, date: str):
    data = schedule_data.get(weak_day)
    for day in data:
        if date in day.get('begin_time'):
            day_sched = day 
    return day_sched.values()


@app.route("/schedule/<string:week_day>/<string:date>")
def get_schedule(week_day, date):
    data = get_day_shedule(schedule_storage.get_data(), week_day, date)
