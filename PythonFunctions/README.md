...
- (extra) Распаковка словарей. Можно передавать словари в качестве именованных аргументов:
    ```python
    kwargs = {"sep": " ! ", "end": "?"} # kwargs рфсшифровывается как key word arguments
    args = [1,2,3,4,5]
...
- Область видимости в программированиие - это места в программе, где доступны объявленные переменные. Локальные переменные доступны только в определенных примерах, например, если мы в файле `1.py` объявим переменную `name = "Alex"`, то в другом файле `2.py` эта переменная доступна не будет. То же самое работает и с функциями, только немного сложнее.
...
- Глобальные переменные - видны во всех функциях и вообще во всем файле, например:
    ...
    print(GLOBAL_R) # 10
    ```
    Наша глобальная переменная `GLOBAL_R` после вызова функции никак не поменялись, хотя в функции мы ей присваивали новое значени `8`. Суть в том, что при таком присваивании в пределах функции у нас создается новая переменная `GLOBAL_R` **локальная** только для функции do_smth, и эта локальная переменная никак не влияет на глобальную. Для того чтобы присвоение в функции влияло на глобальную переменную существует ключевое слово `global`, используется оно следующим образом:
    ```python
    GLOBAL_R = 10 
